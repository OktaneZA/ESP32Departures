...
| DISP-19 | On the Tube board the line is named once in the header tag ("TUBE VICTORIA"), so the row's route column carries the **direction** instead, abbreviated to fit its 52 px: `N/B`, `S/B`, `E/B`, `W/B`, `P2` where TfL gives only a platform number, `IN`/`OUT` for the Hainault loop's Inner and Outer Rail, `NBF`/`SBF` for Harrow-on-the-Hill's fast platforms, and `N/S` for Chesham's single platform serving both ways. `Westbound Platform 26` (Waterloo & City) drops the platform number and reads `W/B`. Each label is measured against the column, which is why the fast platforms are not `N/BF` — that renders 50 px wide and the column is unclipped, so it would run into the destination. Anything unforeseen still falls back to the first three characters |
